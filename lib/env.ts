...
const envSchema = z.object({
  ...
  NEXT_PUBLIC_APP_URL: z
    .string()
    .url()
    .min(1, 'NEXT_PUBLIC_APP_URL is required for payment redirects and OpenAPI generation'),
  NEXT_PUBLIC_APP_NAME: z.string().default('Architect Platform'),
  ALLOWED_ORIGINS: z.string().optional(),

  // Analytics Configuration
  ANALYTICS_PROVIDER: z.enum(['noop', 'google', 'posthog']).default('noop'),
  ANALYTICS_SAMPLE_RATE: z.string().default('1.0'),
});
...
function validateEnv(): Env {
  ...
  if (nodeEnv === 'test') {
    return {
      ...
      GITHUB_ACCESS_TOKEN: process.env.GITHUB_ACCESS_TOKEN || 'test-github-token',
      ...
      ANALYTICS_PROVIDER: process.env.ANALYTICS_PROVIDER || 'noop',
      ANALYTICS_SAMPLE_RATE: process.env.ANALYTICS_SAMPLE_RATE || '1.0',
    } as Env;
  }
