 ...
 * to protect internal system performance data from unauthorized access.
 */
...
export const GET = APIRouteHandler.createCachedGETHandler(
  {
    ...
    handler: async ({ req }) => {
      ...
      if (optimize) {
        await performanceOptimizationService.applyAutoOptimizations(performanceReport);
        response.optimization.autoOptimizations = true;
      }

      return detailed ? response : performanceOptimizationService.simplifyResponse(response);
    },
  },
  {
    ttl: 60,
