...
const WebhookQueueMonitor = lazy(() =>
  import('@/components/monitoring/webhook-queue-monitor').then(module => ({
    default: module.WebhookQueueMonitor,
  }))
);

const WebhookConfigurationManager = lazy(() =>
  import('@/components/webhooks/webhook-configuration-manager').then(module => ({
    default: module.WebhookConfigurationManager,
  }))
);
...
export default function WebhookMonitoringPage() {
  ...
  useState(() => {
    analytics.pageView({ path: '/dashboard/webhooks', title: 'Webhooks Page' });
  });
  ...
  const handleManualRefresh = () => {
    analytics.trackButtonClick('webhook_manual_refresh', 'webhooks', {
      tab: activeTab,
    });
    setLoading(true);
  };
  ...
  const handleTabChange = (tab: TabType) => {
    analytics.trackButtonClick('webhook_tab_click', 'webhooks', {
      tab,
    });
    setActiveTab(tab);
  };
  ...
  const toggleAutoRefresh = () => {
    analytics.trackButtonClick('webhook_auto_refresh_toggle', 'webhooks', {
      enabled: !autoRefresh,
    });
  ...
  const handleQuickAction = (action: string) => {
    analytics.trackButtonClick(`webhook_${action}_click`, 'webhooks', {
      tab: activeTab,
    });
  };
  ...
  return (
    <DashboardLayout>
      <div className="max-w-7xl mx-auto space-y-6">
          ...
          {activeTab === 'monitor' && (
            <>
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                  ...
                  <div className="space-y-3 text-sm text-gray-600">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <strong className="text-gray-900">Queue Ingestion:</strong>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <strong className="text-gray-900">Processing Pipeline:</strong>
                        <span className="text-gray-600 ml-2">
                          Events are processed with retry logic and error handling
                        </span>
                      </div>
                    </div>
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <strong className="text-gray-900">Dead Letter Queue:</strong>
                        <span className="text-gray-600 ml-2">
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                  ...
                  <div className="space-y-2 text-sm text-gray-600">
                    <div className="flex items-start gap-2">
                      ...
                      <span>Monitor queue size to detect processing bottlenecks</span>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-2 h-2 bg-blue-500 rounded-full mt-1.5"></div>
                      <span>Watch dead letter queue for recurring failures</span>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-2 h-2 bg-blue-500 rounded-full mt-1.5"></div>
                      <span>Set up alerts for high error rates or queue overflow</span>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-2 h-2 bg-blue-500 rounded-full mt-1.5"></div>
                      <span>Test webhook endpoints regularly to ensure reliability</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-blue-50 rounded-lg p-6">
                <h4 className="text-sm font-medium text-blue-900 mb-4">Quick Actions & Links</h4>
                <div className="flex flex-wrap gap-2">
                  <button
                    onClick={() => handleQuickAction('view_raw_api')}
                  ...
                  <button
                    onClick={() => handleQuickAction('circuit_breaker')}
                    className="px-3 py-1 bg-blue-600 text-white text-sm rounded hover:bg-blue-700 transition-colors"
                  >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div className="text-center">
                      <div className="text-2xl font-bold text-green-600">
                        {currentStats.queue.processingStats.processedEventsCount}
                      </div>
          ...
          {activeTab === 'history' && (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <div className="text-center py-12">
                ...
                <button
                  onClick={() => {
                    handleQuickAction('manage_webhooks');
                    setActiveTab('configure');
                  }}
        ...
        {/* Footer */}
        <div className="mt-8">
          <DashboardFooter loading={loading} autoRefresh={autoRefresh} lastRefresh={lastRefresh} />
        </div>
      </div>
    </DashboardLayout>
  );
