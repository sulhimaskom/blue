...
// CI BUILD CONFIG: Skip auth-dependent pages
...
// CI OVERRIDE: Skip pre-rendering of pages requiring Clerk
const ciNextConfig = {
  ...nextConfig,
  experimental: {
    ...nextConfig.experimental,
    // Skip ISR and pre-rendering for auth pages in CI
    isrMemoryCacheSize: 0,
  },
  // Skip generation of problematic routes in CI
  output: 'export',
  trailingSlash: true,
  images: {
    unoptimized: true,
...
const finalConfig = process.env.NODE_ENV === 'production' && 
                    !process.env.NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY?.includes('skip') ? 
                    ciNextConfig : nextConfig;
